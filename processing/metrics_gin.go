...
// This file is from https://github.com/zsais/go-gin-prometheus/blob/master/middleware.go
// It has been slightly modified in order to fix errors messages like:
// - request_duration_seconds could not be registered in Prometheus
// - response_size_bytes could not be registered in Prometheus
...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if err != nil {
		p.Logger.Error().Err(err).Msg("Error trying to build http request to send to push gateway")
		return
	}
	client := &http.Client{}
	if _, err = client.Do(req); err != nil {
		p.Logger.Error().Err(err).Msg("Error sending to push gateway")
	}
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			...
			if !ok {
				p.Logger.Error().Err(err).Msgf("%s could not be registered in Prometheus", metricDef.Name)
			}
		}
		switch metricDef {
		case reqCnt:
